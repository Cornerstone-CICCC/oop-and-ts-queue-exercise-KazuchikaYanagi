...
function processReturns(queue) {
  ...
  while (!queue.isEmpty()) {
    const tempReturn = queue.dequeue();

    for (let i = 0; i < tempReturn.books.length; i++) {
      if (
        tempReturn.books[i].daysLate > 0 &&
        !tempQueue.items.includes(tempReturn)
      )
        tempQueue.enqueue(tempReturn);
    }
  }
